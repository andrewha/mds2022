...
int main()
{
    ...
    std::cout << "\n\033[33mContinue test on a buffer of booleans\033[0m\n";
    std::cout << "\n\033[33mStage 7. Copy assignment operation\033[0m\n";

    sz = 1;
    std::cout << "\n1. Instantiate a buffer of size " << sz << " and push " << sz << " `true` to it\n";
    RingBuffer<bool> rb5(sz);
    rb5.push(true);    
    sz = 3;
    std::cout << "\n2. Instantiate another buffer of size " << sz << " and push " << sz << " `false` to it\n";
    RingBuffer<bool> rb6(sz);
    rb6.push(false);
    rb6.push(false);
    rb6.push(false);
    std::cout << "\n3. Assign the second buffer to the first buffer and check if the assignment was successful\n";
    std::cout << "Read element(s) from the second buffer before assignment: ";
    while (!rb6.isEmpty())
        std::cout << rb6.pop();
    std::cout << std::endl;
    rb6 = rb5;
    std::cout << "Read element(s) from the second buffer after assignment: ";
    while (!rb6.isEmpty())
        std::cout << rb6.pop();
    std::cout << std::endl;
